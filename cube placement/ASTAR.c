...
int accepted_heuristic_function(int pos2[N][2])
{
	/*initialize h(n) which must not be greater than the real cost g(n) 
	of the route h(n) is the minimum cost still required for the route,
	in our case and based on the data the minimum real cost is equal to 0.5, 
	but the estimated cost cannot exceed this 
	value and for this reason I set it as h(n)=0.4*/
	double cost_hn = 0.0;
	int i;

	/*the numbered cubes 1-K*/
    for(i=0;i<K;i++) { 
       if (pos2[i][0] != (i+1) || pos2[i][1] != 1){ /*if for y=1 the 1-K cubes are not in the positions 1-Ê*/ 
           cost_hn = cost_hn + 0.4;
       }
    }
    ...
    for(i=K;i<2*K;i++){ 
       if (pos2[i][0] != (i-K+1) || pos2[i][1] != 2){ /*if for y=2 the (Ê+1)-2Ê cubes are not in the accepted positions (K+1)-2K*/ 
           cost_hn = cost_hn + 0.4; 
       }
    }
    ...
    for(i=2*K;i<3*K;i++){  
       if (pos2[i][0] != (i-2*K+1) || pos2[i][1] != 3){ /*if for y=3 the (2Ê+1)-3Ê cubes are not in the accepted positions (2K+1)-3K*/ 
           cost_hn = cost_hn + 0.4;  
    	}
    }
...
{   
    ...
    smaller_cost=st->cost + accepted_heuristic_function(st->pos); /* smaller_cost --> initialization, most min[g(n)+h(n)]*/
    best_comb=st; /*the situation with the smallest cost --> initialization*/
    while(st->next_parent!=NULL){   
		cur=st;
        st=st->next_parent;
        if (st->cost + accepted_heuristic_function(st->pos) < smaller_cost){   
			smaller_cost=st->cost + accepted_heuristic_function(st->pos);  /* smaller_cost --> update, most min[g(n)+h(n)]*/
